...
class CustomSelect extends React.Component {
  ...
  handleOptionClick = e => {
    this.setState({
      defaultSelectText: (e.target.getAttribute("data-name") !== 'No') ? e.target.getAttribute("data-name") : 'Sort by',
      showOptionList: false
    });
    console.log(e.target.innerText.toLowerCase())
    this.props.handleFunction(e.target.innerText.toLowerCase())
  };
        ...
        {showOptionList && (
          <ul className="select-options">
            {optionsList.map(option => {
              return (
                <li
                  className={` custom-select-option ${option.name === defaultSelectText || defaultSelectText === 'Sort by' && option.name === 'No'? 'active' : ''}`}
